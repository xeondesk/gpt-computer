"""
The `learning` module is designed to facilitate the collection and storage of user feedback on the outputs generated by the GPT Computer tool. It provides mechanisms for obtaining user consent, capturing user reviews, and storing this information for future analysis and enhancement of the tool's performance.
...
Learning : dataclass
    Encapsulates the metadata and feedback collected during a session of using the GPT Computer tool, including the prompt, model, temperature, configuration, logs, session identifier, user review, and timestamp.
...
ask_collection_consent() -> bool
    Prompts the user for consent to store their data for the purpose of improving GPT Computer.
...
class Learning:
    """
    A dataclass that encapsulates the learning data collected during a GPT Computer session.

    Attributes
    ----------
    prompt : str
        A JSON string representing the prompt provided to GPT Computer.
    model : str
...
def human_review_input() -> Optional[Review]:
    ...
    print()
    print(
        colored("To help gpt-computer learn, please answer 3 questions:", "light_green")
    )
    print()
...
def ask_collection_consent() -> bool:
    """
    Asks the user for their consent to store their data for the purpose of improving the GPT Computer tool.
    ...
    """
    answer = input(
        "Is it ok if we store your prompts to help improve GPT Computer? (y/n)"
    )
    ...
    if answer.lower() == "y":
        ...
        print(
            colored(
                "No worries! GPT Computer will not collect your prompts. ❤️",
                "light_green",
            )
        )
        return False
...
def extract_learning(
    ...
    ----------
    prompt : str
        The initial prompt provided to the GPT Computer.
    model : str
        The name of the model used during the session.
    temperature : float
        The temperature setting used for the model's responses.
...
def get_session() -> str:
    ...
    str
        A unique identifier for the user session.
    """
    path = Path(tempfile.gettempdir()) / "gpt_computer_user_id.txt"
