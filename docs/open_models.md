...
You can integrate `gpt-computer` with open-source models by leveraging an OpenAI-compatible API.
...
Then you call `gpt-computer` with your service endpoint `--azure https://aoi-resource-name.openai.azure.com` and set your deployment name (which you created in the Azure AI Studio) as the model name (last `gpt-computer` argument).

Example:
`gpt-computer --azure https://myairesource.openai.azure.com ./projects/example/ my-gpt4-project-name`
