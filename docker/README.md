...
This guide provides step-by-step instructions on how to set up and run the Docker environment for your GPT-Computer project.
...
2. **Build the Docker Image**
    ...
    ```bash
    docker build --rm -t gpt-computer -f docker/Dockerfile .
    ```

3. **Run the Docker Container**

    ```bash
    docker run -it --rm -e OPENAI_API_KEY="YOUR_OPENAI_KEY" -v ./your-project:/project gpt-computer
    ```
...
2. **Build and Run using Docker Compose**
    ...
    docker-compose run --rm gpt-computer
    ```
...
```bash
docker run -it --entrypoint /bin/bash gpt-computer
```
