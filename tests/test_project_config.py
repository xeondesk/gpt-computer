...
from gpt_computer.core.project_config import (
    Config,
    _GptComputerAppConfig,
    _OpenApiConfig,
    example_config,
    filter_none,
)
...
def test_config_load():
    ...
    assert config.gptcomputer_app
    assert config.gptcomputer_app.project_id == "..."
    assert config.gptcomputer_app.openapi
    assert (
        config.gptcomputer_app.openapi[0].url
        == "https://api.gptcomputer.app/openapi.json"
    )
    assert (
        config.gptcomputer_app.openapi[1].url
...
def test_config_from_dict():
    d = {"gptcomputer-app": {"project_id": "..."}}  # minimal example
...
""".strip()
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
        f.write(example_config)
    config = Config.from_toml(f.name)
    config.gptcomputer_app = _GptComputerAppConfig(
        project_id="...",
        openapi=[_OpenApiConfig(url="https://api.gptcomputer.app/openapi.json")],
    )
    config.to_toml(f.name)
    assert Config.from_toml(f.name) == config
